...
def parse_args():
    ...
    parser.add_argument(
        '--mesh_reg_checkpoint',
        type=str,
        default='data/checkpoints/expose.pth',
        help='Checkpoint file for mesh regression')
    parser.add_argument('--cam-id', type=str, default='0')
    parser.add_argument(
        '--det_config',
        type=str,
        default='mmdetection/ssdlite_mobilenetv2_scratch_600e_coco.py')
    parser.add_argument(
        '--det_checkpoint',
        type=str,
        default='mmdetection/ssdlite_mobilenetv2_scratch_600e_coco_20210629_110627-974d9307.pth')
    parser.add_argument(
        '--det_cat_id',
        type=int,
        default=1,
    ...
    while True:
        with stop_watch.timeit('_FPS_'):
            ...
            if mesh_results:
                ...
                img = renderer(verts, img, alpha=0.9)
...
def main():
    ...
    body_model = build_body_model(
        dict(
            type=os.path.basename(os.path.normpath(args.body_model_dir)).upper(),
            gender='neutral',
            num_betas=10,
    ...
    # (Optional) filter certain body parts
    # renderer.load_vertices_filter("UPBODY", "HEAD", "ARM")
